...
static int __init ipe_init(void)
{
	...
	for (i = 0; i < ipe_property_max; ++i)
		if (!properties[i].init())
			panic("IPE Failed to Initialize Properties...");

	/* Make Sure the Match Table is NULL Terminated per API definition */
	IPE_REGISTER_PROPERTY(max, NULL);
