				   ...
				   int budget)
{
	...
	struct nvsp_message *init_pkt;

	/* Check if this is a VMBUS completion without data payload */
	if (!msglen) {
		init_pkt = (struct nvsp_message *)desc->trans_id;
		switch (init_pkt->hdr.msg_type) {
