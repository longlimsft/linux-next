...
static int hv_pci_start_relations_work(struct hv_pcibus_device *hbus, struct hv_dr_state *dr)
{
	struct hv_dr_work *dr_wrk;
	bool pending_dr;
	unsigned long flags;
				   ...
				   struct pci_bus_relations2 *relations)
{
	...
	for (i=0; i<dr->device_count; i++) {
		...
		dr->func[i].virtual_numa_node = relations->func[i].virtual_numa_node;
	}
				   ...
				   struct pci_bus_relations *relations)
{
	...
	for (i=0; i<dr->device_count; i++) {
		...
		dr->func[i].win_slot = relations->func[i].win_slot;
		dr->func[i].ser = relations->func[i].ser;
		// TODO how about relations2? set flags and virtual_numa_node
	}
