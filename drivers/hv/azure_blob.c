...
{
	atomic_inc(&az_blob_dev.file_count);
	if (az_blob_dev.removing) {
		if (atomic_dec_and_test(&az_blob_dev.file_count))
			wake_up(&az_blob_dev.wait_files);
		return -ENODEV;
	}

	file->private_data = &az_blob_dev;
...
{
	struct az_blob_device *dev = filp->private_data;
...
static int az_blob_show_pending_requests(struct seq_file *m, void *v)
{
	...
	spin_lock_irqsave(&az_blob_dev.vsp_pending_lock, flags);
	list_for_each_entry(request_ctx, &az_blob_dev.vsp_pending_list, list) {
	...
	spin_unlock_irqrestore(&az_blob_dev.vsp_pending_lock, flags);
...
static int az_blob_create_device(struct az_blob_device *dev)
{
	...
	atomic_set(&az_blob_dev.file_count, 0);
	init_waitqueue_head(&az_blob_dev.wait_files);
...
static int az_blob_connect_to_vsp(struct hv_device *device, u32 ring_size)
{
	...
	spin_lock_init(&az_blob_dev.vsp_pending_lock);
	INIT_LIST_HEAD(&az_blob_dev.vsp_pending_list);
	init_waitqueue_head(&az_blob_dev.wait_vsp);
...
static void az_blob_remove_vmbus(struct hv_device *device)
{
	...
	if (!list_empty(&dev->vsp_pending_list)) {
		spin_unlock_irqrestore(&dev->vsp_pending_lock, flags);
		az_blob_dbg("wait for vsp_pending_list\n");
