...
//	desc.rangecount = pagecount; // set by the caller
	...
	printk(KERN_ERR "%s: desc descsize %u dataoffset8 %u length8 %u rangecount %u\n", __func__, desc_size, desc->dataoffset8, desc->length8, desc->rangecount);
	for (i=0; i<desc->rangecount; i++)
		printk(KERN_ERR "%s: range %d len %u offset %u pfn %llu\n", __func__, i, desc->range[i].len, desc->range[i].offset, desc->range[i].pfn);
