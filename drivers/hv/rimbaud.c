	...
	struct rimbaud_device *dev)
{
	...
	response = kzalloc(user_response_size, GFP_KERNEL);
	if (!response)
		return -ENOMEM;
...
bool check_blob_name_range(u32 name_offset, u32 name_len, size_t max_length)
{
	if (name_len > XS_VSC_MAX_BLOB_NAME_LENGTH) {
		rimbaud_err("name length %d exceeds max length\n", name_len);
		return false;
	}

	if (name_offset + name_len > max_length) {
		rimbaud_err("name range offset %d length %d exceeds max length\n", name_offset, name_len);
		return false;
	}
...
bool check_session_token_range(u32 token_offset, u32 token_len, size_t max_length)
{
	if (token_len > XS_VSC_MAX_SESSION_TOKEN_LENGTH) {
		rimbaud_err("token length %d exceeds max length\n", token_len);
		return false;
	}

	if (token_offset + token_len > max_length) {
		rimbaud_err("token range offset %d length %d exceeds max length\n", token_offset, token_len);
		return false;
	}
	...
	struct rimbaud_device *dev)
{
	...
	if (!check_session_token_range(readblob.session_token_offset, token_len, count))
		return -EINVAL;
	...
	get_blob_vsp = kzalloc(sizeof(*get_blob_vsp) + token_len, GFP_KERNEL);
	if (!get_blob_vsp) {
		rc = -ENOMEM;
		goto alloc_failure;
	}
	...
	request = kzalloc(sizeof(struct rimbaud_request), GFP_KERNEL);
	if (!request) {
		rc = -ENOMEM;
		goto alloc_failure;
	}
	...
	request->vsp_response_packet =
		kzalloc(sizeof(struct rimbaud_get_blob_response), GFP_KERNEL);
	...
	struct rimbaud_device *dev)
{
	...
	// validate token in the buffer
	if (!check_session_token_range(metadata_hint.session_token_offset, token_len, count))
		return -EINVAL;
	...
	// prepare VSP message
	metadata_hint_vsp = kzalloc(sizeof(*metadata_hint_vsp) + token_len,
				GFP_KERNEL);
	...
	struct rimbaud_device *dev)
{
	...
	if (!check_blob_name_range(openblob.blob_name_offset, name_len, count) ||
	    !check_session_token_range(openblob.session_token_offset, token_len, count))
		return -EINVAL;
	...
	open_blob_vsp = kzalloc(sizeof(*open_blob_vsp) + token_len + name_len,
				GFP_KERNEL);
	if (!open_blob_vsp)
		return -ENOMEM;
...
static int rimbaud_connect_to_vsp(struct hv_device *device, u32 ring_size)
{
	...
	hash_init(rimbaud_dev.read_blob_hash);
	init_waitqueue_head(&rimbaud_dev.wait_remove);
	atomic_set(&rimbaud_dev.vsp_pending, 0);
	rimbaud_dev.removing = false;
	spin_lock_init(&rimbaud_dev.read_blob_hash_lock);
...
static int rimbaud_negotiate_version(struct rimbaud_device *dev)
{
	...
	static struct rimbaud_request request;
...
/* FIXME the host returns empty version (0,0) at this time, just pretend the version is negotiated OK
