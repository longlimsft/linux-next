...
static void rimbaud_on_channel_callback(void *context)
{
	...
	foreach_vmbus_pkt(desc, channel) {
		...
		rimbaud_dbg("got response for request %pUb status %u response_len %u\n",
			&request_ctx->request->guid, response->error, response->response_len);
...
{
	return file->f_cred == current_cred() && segment_eq(get_fs(), USER_DS);
}
	...
	struct page ***ppages, size_t *start, size_t *num_pages)
{
	...
	*num_pages = (result + *start + PAGE_SIZE - 1) / PAGE_SIZE;
	...
	rimbaud_dbg("num_pages %lu\n", *num_pages);
	for (i=0; i<*num_pages; i++)
		rimbaud_dbg("*pages[%d] %px pfn %lu\n", i, pages[i], page_to_pfn(pages[i]));
...
//	return -EINVAL;
	...
	desc_size = sizeof(struct vmbus_packet_mpb_array) + sizeof(u64) * total_num_pages;
	...
	for (i=0; i<total_num_pages; i++)
		rimbaud_dbg("i=%d pfn=%llu\n", i, desc->range.pfn_array[i]);
...
get_user_page_failed:
	printk(KERN_ERR "%s: processing get_user_page_failed\n", __func__);
	printk(KERN_ERR "%s: free request pages \n", __func__);
	free_buffer_pages(request_num_pages, request_pages);
	printk(KERN_ERR "%s: free response pages \n", __func__);
	free_buffer_pages(response_num_pages, response_pages);
	printk(KERN_ERR "%s: free data pages \n", __func__);
