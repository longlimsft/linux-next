...
{
	unsigned long buffer_addr = (unsigned long) buffer;
	int ret;
	unsigned long offset = buffer_addr & ~PAGE_MASK;
	unsigned int npages =
		(offset + buffer_len + PAGE_SIZE - 1) >> PAGE_SHIFT;

	*pages = kzalloc(npages * sizeof(struct page *), GFP_KERNEL);
	if (*pages)
		return -ENOMEM;

	ret = get_user_pages_fast(buffer_addr, npages, 1, *pages);
	if (ret != npages) {
		rimbaud_err("can't ping user pages requested %u got %d\n",
			npages, ret);
		return -EFAULT;
	}
