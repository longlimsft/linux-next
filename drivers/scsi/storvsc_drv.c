...
static int storvsc_queuecommand(struct Scsi_Host *host, struct scsi_cmnd *scmnd)
{
	...
	if (sg_count) {
		...
		for (i = 0; sgl != NULL; sgl = sg_next(sgl)) {
			 ...
			 * hvpgoff will be zero except for first sgl entry.
			 */
			sgl_size = PAGE_ALIGN(sgl->offset +
					 sgl->length) >> HV_HYP_PAGE_SHIFT;
			hvpgoff = sgl->offset >> HV_HYP_PAGE_SHIFT;
			hvpfn = page_to_hvpfn(sg_page(sgl));

			/*
			 * Fill a portion of the PFN array with sequential
			 * Hyper-V PFNs for the continguous physical
			 * memory described by the sgl entry. Quit if the
			 * PFN array is filled.
			 */
